...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    // Add log event - IMPROVED SUCCESS MESSAGE
    const resourceId = rows[0].id;
    await logEvent({
      actorUserId,
      message: `RESOURCE_CREATED: name="${resourceName}", id=${resourceId}, price=${resourcePrice}/${resourcePriceUnit}`,
      entityType: "resource",
      entityId: resourceId,
    });
    ...
    if (err && err.code === "23505") {
      // Add log event - IMPROVED DUPLICATE MESSAGE
      console.error(err);
      await logEvent({
        actorUserId,
        message: `RESOURCE_CREATE_FAILED_DUPLICATE: name="${resourceName}" already exists (409)`,
        entityType: "resource",
        entityId: null,
      });
