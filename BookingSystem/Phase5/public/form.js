  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400) {
        ...
        let validationMsg = "❌ Please fix these issues:\n\n";
        
        errors.forEach(error => {
          if (error.field === 'resourceName') {
            validationMsg += "• Resource name: Use 5-30 characters (letters, numbers, and spaces only)\n";
          } else if (error.field === 'resourceDescription') {
            validationMsg += "• Description: Use 10-50 characters (letters, numbers, and spaces only)\n";
          } else if (error.field === 'resourcePrice') {
            validationMsg += "• Price: Must be a valid number (0 or greater)\n";
          } else {
            validationMsg += `• ${error.field}: ${error.msg}\n`;
          }
        });
        
        validationMsg += "\n✨ Fix the highlighted fields and try again!";
        
        showFormMessage("error", validationMsg);
        return;
      }

      // 409 = duplicate resourceName (our new feature)
      if (response.status === 409) {
        // IMPROVED DUPLICATE MESSAGE
        const resourceName = payload.resourceName;
        const msg = `⚠️ Cannot create "${resourceName}"\n\n` +
          `A resource with this name already exists in the system.\n\n` +
          `💡 What you can do:\n` +
          `• Choose a different name (e.g., "${resourceName} 2" or "${resourceName} - Main")\n` +
          `• Check the resources list to see existing names\n` +
    ...
    const resourcePrice = body?.data?.price ?? "";
    const resourceUnit = body?.data?.price_unit ?? "";

    // Format the date nicely
    const createdDate = body?.data?.created_at 
      ? new Date(body.data.created_at).toLocaleString() 
      : new Date().toLocaleString();

    const successMessage = `✅ Resource "${resourceName}" created successfully!\n\n` +
      `• ID: ${resourceId}\n` +
      `• Price: €${resourcePrice} per ${resourceUnit}\n` +
      `• Created: ${createdDate}\n\n` +
      `The resource is now available for booking.`;
